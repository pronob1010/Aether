...
`Aether` is the low-level client you *call*; an `Agent` is a configuration you
...
from aether.config import get_max_delegation_depth
from aether.llm.contracts import LLMProvider, LLMResponse, Message
from aether.middleware import Middleware
from aether.tools import register_tool
...
_delegation_depth: ContextVar[int] = ContextVar("aether_delegation_depth", default=0)
...
class Agent:
    ...
    def __init__(
        ...
        *,
        ...
        if client is not None:
            ...
            self.client = Aether(provider, middleware=middleware)
        else:
            self.client = Aether(middleware=middleware)
    ...
    def as_tool(
        ...
        *,
        ...
        client dispatches them concurrently (see `Aether.complete`).

        Delegation is bounded: once `max_depth` nested levels are reached
        (default from `AETHER_MAX_DELEGATION_DEPTH`), a further delegation is
