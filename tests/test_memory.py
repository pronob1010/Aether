"""Session memory subsystem — stateful conversations via Agartha.session()."""
import asyncio
import pytest
from agartha import Agartha, Message, Session, register_tool
...
async def test_history_persists_across_client_restart():
    """A new Agartha sharing the same store sees the prior session's history."""
    store = InMemorySessionStore()
    client_a = Agartha(FakeProvider(canned_response="reply"), memory_store=store)
    session_a = client_a.session("z")
    await session_a.ask("first")

    # Simulate a new process — fresh client, same store
    client_b = Agartha(FakeProvider(canned_response="other"), memory_store=store)
...
async def test_custom_store_works_via_protocol():
    ...
    client = Agartha(FakeProvider(canned_response="ok"), memory_store=store)
