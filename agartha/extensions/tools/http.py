...
optional host allowlist. See `agartha.config` for the gates. Note this is a
...
def _ssrf_check(url: str) -> str | None:
    ...
    if allowed is not None and host not in allowed:
        return f"Error: host {host!r} is not in AGARTHA_HTTP_TOOL_ALLOWED_HOSTS."
    ...
    for info in infos:
        ...
        if (
            ...
            return (
                ...
                f"AGARTHA_HTTP_TOOL_ALLOW_PRIVATE=1 to allow."
            )
    return None
...
async def http_get(url: str, timeout: float | None = None) -> str:
    ...
    Args:
        ...
        timeout: Request timeout in seconds. Omit to use the framework
            default (AGARTHA_HTTP_TOOL_TIMEOUT env, falls back to 10s).
    """
