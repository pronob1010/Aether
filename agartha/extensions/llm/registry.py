"""LLM-specific facade over the generic `agartha.registry`.
...
The actual storage lives in `agartha.registry.REGISTRY`. Tests, factories,
...
def register_provider(name: str, **metadata):
    ...
    consumed by `Agartha()` when auto-detecting from env:
...
def _load_openai():
    from agartha.extensions.llm.openai import OpenAIProvider
    return OpenAIProvider

def _load_gemini():
    from agartha.extensions.llm.gemini import GeminiProvider
    return GeminiProvider

def _load_anthropic():
    from agartha.extensions.llm.anthropic import AnthropicProvider
