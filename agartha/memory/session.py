...
Wraps an `Agartha` client and a `SessionStore`. Each `ask` / `complete` /
...
Correctness rules baked in:
  1. Each turn passes a COPY of the history to `Agartha.complete()`,
...
from agartha.llm.contracts import LLMResponse, LLMStreamChunk, Message
...
class Session:
    """One stateful conversation. Created via `Agartha.session(id)`."""
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        async with self._lock:
            ...
            try:
                # Copy so Agartha's internal tool loop doesn't mutate our list.
